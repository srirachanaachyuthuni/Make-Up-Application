...
class DetectLandmarks(object):
    ...
    def get_eyes(self, image_file, flag=None):
        """
        Returns points for upper eyelids in given image.
        ________________________________________________
        Args:
            ...
            2. `flag`:
                ...
                By default its value is IMAGE_DATA, and assumes imread() image is passed.
        Returns:
            String with list of detected points of eyes.
        Error:
            Returns `None` if face not found in image.
        """
        landmarks = self.get_face_data(image_file, flag)
    ...
    def get_cheeks(self, image_file, flag = None):
        """
        Returns points for the cheek bones' blush area.
        Args:
            1. `image_file`:
                Either of three options:\n
                    a. (int) Image data after being read with cv2.imread()\n
                    ...
                    c. Byte stream being received over multipart network reqeust.\n\n
        Returns:
            String with list of detected points of cheeks.
        Error:
            Returns `None` if face not found in image.
        """
        landmarks = self.get_face_data(image_file, flag)
        left_cheek = [landmarks[36], landmarks[39], landmarks[31], landmarks[48], landmarks[2]]
        right_cheek = [landmarks[42], landmarks[45], landmarks[14], landmarks[54], landmarks[35]]
        mid_point_left_cheek = get_midpoint(left_cheek)
        mid_point_right_cheek = get_midpoint(right_cheek)
        cheek_midpoint = str(mid_point_left_cheek).replace('[', '').replace(']', '') + '\n' + str(mid_point_right_cheek).replace('[', '').replace(']', '')
        return cheek_midpoint
